...
static const Uint8 dir_bytes[15] = {8,0,0,0x53,0xae,0x93,0x4a,0x93,0x4a,0,0,0x53,0xae,0x93,0x4a};
static const Uint8 default_volume_name[11]	= "M.E.G.A.65!";
static const char  default_disk_image[]		= "mega65.d81";
...
// Actually this only makes the MBR and parition boundaries, and calls mkfs_fat32() for the filesystem creation on a parition.
// I copied the algorithm from mega65-fdisk closely (and hopefully well enough) not to have any compatibility issue.
static int fdisk ( Uint32 device_size )
{
	...
	if (mksyspart(sys_part_start, sys_part_sects))
		return -1;
	// *** Make fat32.c to re-read parition table ...
...
static int check_filename_syntax ( const char *fn )
{
	const char *p1 = strchr(fn, '.');
	const char *p2 = strrchr(fn, '.');
	if ((p1 && p2 && p1 != p2) || (p1 == fn) || (p1 && !p1[1]))
		return 1;
}



// Put file onto the SDcard image, with or without overwriting the previous one (if any).
// The directory will be used which is "in use", path is not supported in "fn".
static int put_file ( const char *fn, const Uint8 *data, int size, int overwrite )
{
}



static int update_commons ( void )
{
	put_file("BANNER.M65",	meminitdata_banner,	MEMINITDATA_BANNER_SIZE,	1);
	put_file("FREEZER.M65",	meminitdata_freezer,	MEMINITDATA_FREEZER_SIZE,	1);
}


static int update_from_directory ( const char *dirname )
{
	...
	for (;;) {
		...
		DEBUGPRINT("Candidate for updating: %s" NL, fn);
		struct stat st;
		stat(fn, &st);
	}
	return errno;
...
#define SDCONTENT_FORCE_FDISK
#define SDCONTENT_ASK_FDISK
...
int sdcontent_handle ( Uint32 size_in_blocks, int force_fdisk, int do_commons, const char *update_dir_path )
{
	...
	if (has_block_nonzero_byte(block)) {
		int ret = mfat_init_mbr();
		DEBUGPRINT("mtr_init__: %d" NL, ret);
		ret = mfat_use_part(0);
		DEBUGPRINT("use_part_0: %d" NL, ret);
	} else {
		DEBUGPRINT("SDCARD: FDISK: forced, since MBR is all zero bytes" NL);
		force_fdisk = 1;
	}
	if (force_fdisk) {
		fdisk(size_in_blocks);
	}
	DEBUGPRINT("Freezer size = %d" NL, meminitdata_freezer_size);

	//if (is_all_zero(block, 512))
	//	force_fdisk = 1;	// enable fdisk, since partition table seems to be all zero
	//if (mfat_init_mbr() 
	// TODO: validate SD-card. If no sane parition found, force "force_fdisk" (or at least ASK user if that's OK to do that ...)
#if 0
	if (ret != 0)	// mfat_init_mbr() should return with zero (zero'th partition is OK), if not, it's lack of suitable partition table
		force_fdisk = 1;
	} else {
		// ... but still we need to check if filesystem is valid as well! Not only the partition table!
		if (mfat_use_part(0))
			....
	}
#endif
	//if (force_fdisk)
	//	fdisk(sdcard_size_in_blocks);
	//ret = mfat_use_part(0);
	//DEBUGPRINT("use_part_0: %d" NL, ret);
#if 0
	if (force_fdisk) {
		if (fdisk(sdcard_size_in_blocks))
			return -1;
	}
#endif
	if (update_dir_path)
		update_from_directory(update_dir_path);
	return 0;
}
