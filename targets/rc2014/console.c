...
{
	static char escape_buffer[100];
	// CRLF?
//	cursor.x = 0;
//	if (cursor.y == console_height - 1)

	if (cursor.escape) {
		int l = strlen(escape_buffer);
		escape_buffer[l++] = data;
		escape_buffer[l] = 0;
	} else if (data == 27) {
		// start of some ESCAPE sequence ...
		cursor.escape = 1;
		escape_buffer[0] = 0;
	} else if (data == 13) {
		cursor.x = 0;
...
int console_init ( int width, int height, int zoom_percent, Uint8 *video_mapped, Uint8 *color_mapped )
{
	...
	video_ram = video_mapped ? video_mapped : xemu_malloc(width * height);
	color_ram = color_mapped ? color_mapped : xemu_malloc(width * height);
	console_width = width;
